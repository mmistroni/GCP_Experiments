...
def run(argv=None, save_main_session=True):
    ...
    with beam.Pipeline(options=pipeline_optionss) as p:
        ...
        senate_disclosures_sink = beam.io.WriteToBigQuery(
            ...
            schema='AS_OF_DATE:DATE,TICKER:STRING,DISCLOSURE:STRING,representative:STRING',
            write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND,
            create_disposition=beam.io.BigQueryDisposition.CREATE_IF_NEEDED)
        ...
        prepped_senate_data = (senate_disc | 'Remapping SD ' >> beam.Map(lambda d: dict(AS_OF_DATE=datetime.strptime(d['AS_OF_DATE'], '%Y-%m-%d').date(),
